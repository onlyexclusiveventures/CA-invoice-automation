...
def get_latest_invoice():
    ...
    if response.status_code == 200:
        ...
        if results:
            ...
            print(f"Fetched invoice: {invoice_page['id']}")
            return invoice_page
    else:
...
def create_invoice(invoice_fields, employee_id, employer_id):
    ...
    data = {
        "parent": {"database_id": OEV_INVOICES_DATABASE_ID},
        "properties": {
            "Name": {"title": [{"text": {"content": invoice_fields.get('Name', 'Untitled')}}]},
            "Status": {"select": {"name": invoice_fields.get('Status', 'Payment Processing')}},
            "Date Issued": {"date": {"start": invoice_fields.get('Date Issued', '')}},
            "Date Due": {"date": {"start": invoice_fields.get('Date Due', '')}},
            "Employee": {"relation": [{"id": employee_id}]},
            "Employer": {"relation": [{"id": employer_id}]},
            "Sum": {"number": invoice_fields.get('Sum', 0)}
        }
    }
    response = requests.post(url, headers=headers, json=data)
    if response.status_code == 200:
...
if invoice_page:
    ...
    if page_id not in processed_pages:
        ...
        if employee_id:
            ...
            save_processed_page(page_id)  # Save the processed page ID
        else:
            print(f"Employee {employee_name} not found!")
    else:
        print(f"Invoice {page_id} has already been processed.")
else:
